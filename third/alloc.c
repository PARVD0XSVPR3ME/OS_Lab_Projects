 ...
 */
  ...
  /*arxikopoihsh bestfit , biggest*/
  phys_clicks old_base,bestfit,biggest;  
  do 
  {
	  /*thetw th metavlith biggest 0 wste na vrw to megalytero hole argotera kai thn
	  bestfit -1 wste na vrw to mikrotero mikos hole argotera*/
	biggest = 0;
	bestfit = -1; 
	prev_ptr = NIL_HOLE;
	hp = hole_head;
	...
	while (hp != NIL_HOLE && hp->h_base < swap_base) {
		...
		/*Diatrexw oles tis diathesimes opes */
		if (hp->h_len >= clicks && hp->h_len < bestfit ) {	
		/*Kai apothikeyw to mikos kai thn arxh ayths poy einai h mikroterh apo tis opes poy einai ises 
		h megaliteres apo thn timh poy zhthsa*/
			old_base = hp->h_base;
			bestfit = hp->h_len;
		}
	...
	/*Arxikopoihsh gia thn 2h while:o deikths hp deixnei pali sthn prwth diathesimh oph kai to prev_ptr 
	to thetw 0*/
	prev_ptr = NIL_HOLE;
	hp = hole_head;
	while (hp != NIL_HOLE && hp->h_base < swap_base) {
		/*Diatrexw oles tis diathesimes opes */
		if (hp->h_len == bestfit) {	
			/*An to mikos ths ophs pou deixnei to hp einai ish me to bestfit 
			Diladh an to mikos ths trexousas ophs einai to zitoumeno pou ikanopoiei ton algorithmo  
			bestfit*/
			...
			/*Arxikopoihsh gia thn 3h while:o deikths hp deixnei pali sthn prwth diathesimh oph kai 
			to prev_ptr to thetw 0*/
			prev_ptr = NIL_HOLE;
			hp = hole_head;
			...
			/*Diatrexw oles tis diathesimes opes */
				...
				if (hp->h_len > biggest ) {
				/*kai apothikevw thn oph me to megalyero mhkos*/
					biggest = hp->h_len;
				}
				prev_ptr = hp;
				hp = hp->h_next;
			...
			printf("\nRequested: %d  Slot used: %d Largest slot: %d\n",clicks,bestfit,biggest);
